...
- **The cost:** $60–120 (MVP, 4–6 weeks) to $80–170 (full round, 7–11 weeks) at Sonnet 4.6 rates per [§17 decision #2](#decisions-outstanding); 5× higher at Opus 4.7 rates if round 2 escalates. Pre-flight + cost-control gates in [§15](#roadmap) cap downside risk.
...
**Scoring rule for restraints (binding under §8's decision hierarchy).** For a recommendation against any restraint plant, the §8 scoring table at the top of "Scoring per recommendation" governs auto-scoring: `no-action` with grounded rationale citing the context signal scores 1.0; `no-action` without specific rationale scores 0.5; any action recommendation scores 0.0 — regardless of whether that action category appears in the plant's `wrong_answers` enumeration. The `wrong_answers` list on restraint plants is *illustrative* — it documents which action categories are most plausible to FP on (typically the canonical's own category) — but the restraint table dominates auto-scoring. Concretely: an agent recommending `protocol-inheritance` on a `default-implementation` restraint scores 0.0 under the restraint table, not "panel-route because the category isn't enumerated in `wrong_answers`."

**Cross-lens restraints (cluster_id shared across plants).** A single source cluster can legitimately serve as the restraint for more than one category — Plants 3R and 5R in the [plant manifest](refactor-recommendation-experiment-plant-manifest.md) are an instance, both pointing at the same `PlaylistTesting/` stub cluster under default-implementation (3R) and generic-parameterization (5R) lenses respectively. Round 2 (#33) resolves this structurally via an explicit `cluster_lens` field on each cluster-sharing plant. The auto-scorer's binding layer (`score_all.bind_recs_to_plants`) applies a post-filter that consults `cluster_lens` to attribute action recommendations to the single lens-matching plant. Three routing branches:

1. **Action rec whose `category` matches exactly one candidate's `cluster_lens`** → bind to that single plant. The lens claim is unambiguous; the bounded round-1 FPR inflation is eliminated for this case.
2. **`no-action` rec** → bind to all candidates. The §9 restraint table scores `no-action` identically across lens-sharing plants (1.0 grounded / 0.5 ungrounded regardless of lens), so each plant's per-cell FPR denominator increments correctly.
3. **Action rec whose `category` matches no candidate's `cluster_lens`** (e.g., `protocol-inheritance` against the 3R+5R shared cluster) → bind to all candidates. The §9 restraint table makes every action recommendation a 0.0 FP regardless of lens; the round-1 fan-out behavior is preserved for this case, with the bounded inflation transparently reported (and per-category breakdowns disaggregating it as before).

The manifest validator (`validate-manifest.py` check 11) enforces that plants in a cross-lens sharing-group declare distinct `cluster_lens` values from `CATEGORIES`; the sharing-group condition is the §9-structural overlap (source_files ∩ + expected_cluster_symbols ∩) restricted to plants with distinct `category` (cross-lens). Same-category cluster-sharing is a separate manifest-design concern outside #33's scope — the auto-scorer's existing per-plant independent scoring handles it under the canonical rubric.
...
flowchart LR
    ...
    D["Phase D<br/>Trial execution<br/>2–5 days<br/>$80–170 / $60–120 MVP<br/>(Sonnet 4.6)"]
...
**Phase D — Trial execution (2–5 days wallclock, API-budget-bound).** Three conditions × 4 trials × 40 plants ≈ 480 recommendations per model tier. With two model tiers, ~960 recommendations. Per-recommendation cost estimate: ~12.5K input tokens (the prompt plus cluster-context for one cluster row) × ~1.5K output tokens (one structured JSON recommendation). At rough Claude API pricing for a Sonnet-tier model (~$3/M input, ~$15/M output as of early 2026 — round 1 commits to Sonnet 4.6 per [§17 decision #2](#decisions-outstanding)), that's ~$0.06 per recommendation. For a single-tier run of 480 recommendations: **~$29 in trial-execution API spend**; the two-tier 960-recommendation variant comes to ~$58. Sub-experiments (panel-reviewed sample, natural-findings round, optional human-expert comparison condition) add roughly another $50–110 of mixed API spend and panel-time dollar-cost (panel time scoped to ~12 person-hours regardless of tier choice, since the sub-experiment sample doesn't grow proportionally with the trial count). **Total Phase D budget: $80–140 (single-tier) or $110–170 (two-tier).** Wallclock is dominated by API rate limits and parallelization choices, not by serial work — a thorough run can complete in 2–5 days. Opus-tier rates are ~5× more expensive across the board — Opus 4.7 at ~$15/M input, ~$75/M output would put the same single-tier run at ~$145 and the two-tier at ~$290 of API spend; the Sonnet pin trades model capability for budget headroom and is reversed in round 2 if S1-vs-S2 deltas are inconclusive and a more capable model would resolve them.
...
1. **Round 1 scope — MVP or full.** [§16 MVP](#mvp) at 25 plants / 5 categories / 2 conditions / $60–120 vs [§15 full round](#roadmap) at 40 plants / 8 categories / 3 conditions / $80–170 (both at Sonnet 4.6 rates; ~5× higher at Opus). The MVP delivers a credible result in ~4–6 weeks; the full round delivers the complete methodology in ~7–11 weeks. **Default if uncontested:** MVP, on the principle that round 1 should validate the methodology cheaply and round 2 broadens. **Decided by:** the person committing the API budget.
