...
- `cluster_lens` (round-2, [#33](https://github.com/jakebromberg/code-audit-pipeline/issues/33)) is the lens-routing field for plants that share a substrate cluster with at least one other plant of a different `category`. Value is drawn from `CATEGORIES` (the MVP refactor-category enum). Required iff the plant is in a cross-lens sharing-group; the validator detects sharing-groups via overlapping `source_files` AND `expected_cluster_symbols` AND distinct `category` values, and enforces both presence (check 11b) and uniqueness within a group (check 11c). The auto-scorer routes action recommendations to the single plant whose lens matches the rec's category; `no-action` recs and outside-lens action recs fall through to round-1 fan-out across all sharing-group members. See [methodology §9](refactor-recommendation-experiment-methodology.md#restraint) for the routing semantics.
