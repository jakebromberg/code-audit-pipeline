...
- **Status:** promoting via #300 (three chained PRs). **Reconciliation with the spec below:** the decided scope narrows this candidate in two ways — (1) string literals emit in the **binding position only** (numerics stay in both positions); string *arguments* are deferred, so the `store.set(_, forKey:)` arg-string synergy with the userdefaults-keytable candidate does **not** land here and its reasoning is moot under this scope; (2) the query is **not** consumed unchanged — it gains an explicit `value_kind` kind-class bucket (so `"2"` ≠ `2`) plus a string deny-list, beyond the "optional" edit anticipated below. Read the sections below as the original captured proposal, superseded on these two points.
