...
**Before-signal:** two non-protocol types share ≥ `min_intersection` field names (default 5, absolute — not just a ratio) AND each side keeps fields the other lacks. The *mutual residue* is what picks this pair's recommendation — extract an interface and keep both types, not merge. The lanes are lenses, not a partition: [`near-duplicates(-any)`](../pipeline/queries/near-duplicates-any.jq) has no residue gate, so a mutual-residue pair whose Jaccard clears its threshold while staying under this detector's 0.9 merge bound surfaces in both queries, and the residue is the tiebreaker arguing extract-interface over merge. Read against the sibling shapes: near-duplicates(-any) flags high-Jaccard pairs (merge), [`subset-pairs`](../pipeline/queries/subset-pairs.jq) flags one-sided residue (composition / lift), and this detector flags mutual residue over a big absolute intersection.

**Detector** (implemented; the full gate list, thresholds, and rationale live in the [query header](../pipeline/queries/shared-interface-candidates.jq) — the single source of truth): the core shape test is a large absolute intersection with mutual residue inside a bounded Jaccard band, plus lane-routing exclusions and two gates that came out of field-testing the query's own output on wxyc-ios-64 (all-conflicting intersections are authored-override vs resolved-value pairs, and containment pairs are wrapper-over-composition — neither wants a protocol). Slot comparison is type-aware: shared names split into `shared_slots` (type text agrees — the protocol-requirement candidates) and `conflicting_slots` (type text differs — machine-visible merge blockers). Pairs whose types already share a non-trivial protocol demote to the tail via `conformance_index`, which counts `extension Foo: P {}` retroactive conformances — so a finding self-extinguishes once the refactor it recommends lands as one-line conformance extensions.
...
**Before-signal:** two predicates over literal-catalog rows, joined on `value_norm` (spellings collapsed: `6.0` / `0xFF` / `1_000` → `6` / `255` / `1000`). Cluster: same (`value_norm`, label) at ≥ `min_sites` occurrences spanning ≥ `min_files` files, where the label is the knob the literal is attached to (`binding_name`, or `arg_label` falling back to `callee`). Pair: two bindings with equal `value_norm`, different file (package-qualified — two package roots can carry the same relative path) or enclosing type, whose names contain one another case-insensitively (contained name ≥ 4 chars) — the single mirrored constant a site-count gate can't see.
