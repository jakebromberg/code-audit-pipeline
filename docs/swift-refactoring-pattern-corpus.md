...
**Detector** (implemented; gates and rationale in the [query header](../pipeline/queries/shared-interface-candidates.jq)): intersection ≥ `min_intersection`, mutual residue on both sides, 0.25 ≤ Jaccard < 0.9, both-protocol pairs excluded (protocol-inheritance-candidates' lane), same-name pairs excluded (shadows' lane), at least one type-agreeing shared slot (an all-conflicting intersection is an authored-override vs resolved-value pair — optional override fields against their resolved forms — a distinct relationship shape with nothing to put in a protocol requirement list), and containment pairs excluded (either side declaring a field typed as the other side marks a wrapper/facade over composition, not two parallel models — both of these gates came out of field-testing the query's own output on wxyc-ios-64). Slot comparison is type-aware: shared names split into `shared_slots` (type text agrees — the protocol-requirement candidates) and `conflicting_slots` (type text differs — machine-visible merge blockers). Pairs whose types already share a non-trivial protocol demote to the tail via `conformance_index`, which merges `extension Foo: P {}` retroactive conformances into the lookup — so a finding self-extinguishes once the refactor it recommends lands as one-line conformance extensions.
