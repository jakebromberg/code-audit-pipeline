...
**Skip rules.** The Swift walker's standard directory skips (dotdirs, `node_modules` / `build` / `dist` / `coverage` / `DerivedData` / `Pods`), plus a post-walk filter on the contract's normative test-path pattern set (see "Test path patterns" above) unless `--include-tests` is passed. Note the polarity: tests are *excluded by default* here, unlike the `type` and `func` catalogs — this is an occurrence catalog that deliberately omits per-row `is_test` tagging, so exclusion has to happen at extraction time instead.
