...
This is not just a count that shifts — the *flagged set itself* can invert, because `ratio_to_median` moves with a codebase-wide median that every file's row count feeds into. Measured on this repo's own Swift extractor sources (`extractors/swift/Sources/swift-catalog`) at `min_decls 4, ratio 1.5`: before this change, the median declaration count across the 7 source files is 11, and two files clear the gate — `PackageGraphExtractor.swift` (28 decls, 2.5x median) and `TypeCatalogVisitor.swift` (22 decls, 2x median). After, every file's decl_count rises because short-body accessors and one-liners now emit rows, and the median rises with them (11 → 23) — neither previously-flagged file clears the gate any longer (`PackageGraphExtractor.swift` falls to 1.2x, `TypeCatalogVisitor.swift` to 1.3x), so the flagged set drops from two files to zero at these thresholds. A team diffing `module-symbol-density.jq` output across this version bump will see modules disappear from (and, at other threshold/codebase combinations, appear in) the flagged set for reasons that have nothing to do with code quality — it is a denominator change, not a code-quality change.
