...
| Go | `*_test.go` |
| Swift | `Tests/` (capital T, SwiftPM convention); `*Tests.swift` (basename suffix, SwiftPM convention). Extractors may extend with AST-based `XCTestCase`-subclass detection. |
...
**Body-fields gating.** Functions whose normalized body has fewer than `--min-body-lines` (default 3) lines emit a row with `body_hash` / `body_lines` / `body_line_count` / `body_length` all `null`. Body-level cluster queries (`function-duplicates.jq`, `generic-function-candidates.jq`, `default-impl-candidates.jq`) early-filter with `select((.body_line_count // 0) >= 3)` — `null // 0` evaluates to `0`, so null-body rows drop out before any `group_by(.body_hash)`. Signature-level queries (`public-api-leaks.jq`) consume rows regardless of body presence — exported one-liner functions still need leak detection.

Extractors that emit a row per declaration regardless of body length (rather than skipping short bodies outright) change the *row count*, not just body-field presence, for any consumer that counts declarations without a body-length gate. `module-symbol-density.jq` is one such consumer: its `decl_count` (and the codebase-wide `median`/`ratio_to_median` derived from it) groups every function + type row with no gate on `body_line_count`, so a file of many one-line accessors goes from ~0 function rows to one row per accessor. This is the intended trade — it matches the TypeScript and Python extractors, which already emit a row per declaration.

This is not just a count that shifts — the *flagged set itself* can invert, because `ratio_to_median` moves with a codebase-wide median that every file's row count feeds into. Measured by running both extractor versions over the **same** target tree (this repo's `extractors/swift/Sources/swift-catalog`, 7 files) at `min_decls 4, ratio 1.5`, so that only the extractor changes: before, the median declaration count is 10 and two files clear the gate — `PackageGraphExtractor.swift` (28 decls, 2.8x median) and `TypeCatalogVisitor.swift` (22 decls, 2.2x median). After, every file's `decl_count` rises because short-body accessors and one-liners now emit rows, and the median rises with them (10 → 22) — both previously-flagged files drop below the ratio gate, and a file that was nowhere near it takes their place: `LiteralCatalogVisitor.swift` goes from 12 decls to 34 and becomes the sole flag, at 1.5x. The flagged set swaps wholesale rather than merely growing. A team diffing `module-symbol-density.jq` output across this version bump will see modules disappear from and appear in the flagged set for reasons that have nothing to do with code quality — it is a denominator change, not a code-quality change.

Reproducing that comparison requires holding the target tree fixed across the two extractor versions. Running each version over its own checkout instead conflates the extractor's behavior change with whatever source edits shipped alongside it — for the change described here, that conflation moves the same measurement to "flagged set drops to zero," which understates the effect and misses the inversion entirely.

Separately, `kind_breakdown` on Swift catalogs is currently unreliable for one specific kind: `visit(VariableDeclSyntax)` labels `willSet`/`didSet` observers on **stored** properties as `kind: "computed-property"` (filed as #327 — the bug is pre-existing, but observer bodies are almost always one or two lines, so before this change the old `--min-body-lines` gate silently dropped nearly every observer row, keeping the mislabel invisible; after this change every observer row lands in the catalog). Until #327 lands, consumers reading `kind_breakdown` or `decl_count` on Swift catalogs will see inflated `computed-property` counts that are actually stored-property observers.

**`enclosing_type` (optional, per-extractor).** Dotted nesting-stack qualification of the function's enclosing declaration, mirroring the literal catalog's `enclosing_type` (see "Literal catalog" below): nested types join with `.` (`Outer.Inner`), extensions carry the extended type's text (`extension Array<Foo>` → `Array<Foo>`), and free functions emit `null`. Currently emitted by the Swift extractor; consumers read it as `.enclosing_type // null` so catalogs and extractors predating the field stay valid. It is the same string `TypeCatalogVisitor.qualify` (Swift) produces for the enclosing declaration, and `(package, enclosing_type)` — not `enclosing_type` alone — is required to join onto a type-catalog row's `(package, name)`: two package roots can carry a same-named (or same-nested-name) type (see "Why file paths are package-qualified" below for the general case), so a bare-`name` join silently conflates unrelated types across packages — the bug class `copied-literal-candidates.jq`'s cluster and pair gates were fixed to avoid by pairing on `package` alongside the value/name key. But `(package, enclosing_type)` is not an exact 1:1 key onto a single type-catalog row — the join is one-to-many and must be disambiguated on `.kind`. A declaration and its extensions share the same `(package, name)`: `protocol Greeter {}` plus `extension Greeter { ... }` in package `Foo` both emit rows keyed `(Foo, "Greeter")`, one `kind: "interface"` and one `kind: "extension"`, so `$type_idx[[pkg, enc]] | .[0]` picks nondeterministically between the declaration and its extension. A match can also be extension-only, with no declaration row at all: `extension Array<Concert>` emits a type-catalog row keyed `(package, "Array<Concert>")` with `kind: "extension"`, and no row for `Array` itself ever appears (it's a stdlib type, never declared in-package) — that extension-only row carries neither `fields` nor `shape_sig`, since those are populated only from properties the extension itself adds.
...
**String values (`value_kind: "string"`).** For a string binding, `value` is the literal's source text *including* its surrounding quotes (`"on_tour_for_you_station_cap"`); `value_norm` is the concatenated content of the string's segments with the quotes removed — verbatim, with **no case-folding and no escape decoding**. Escapes stay literal: `"a\tb"` normalizes to the four characters `a\tb` (backslash-`t`), not a tab. Determinism is all the join key needs — two identically-written literals produce the same `value_norm` — and case is preserved so `"Foo"` and `"foo"` do not collide. Because the numeric normalizations above (radix, float collapse) are meaningless for strings, a string `value_norm` is never cross-spelling-folded. Consuming queries **must** therefore separate kinds before clustering or pairing — treating `int`/`float` as one class and `string` as another — so a string never unifies with a numeric of the same text (`"2"` ≠ `2`). (The current numeric-only query trivially satisfies this; the copied-literal query adds an explicit `value_kind` kind-class bucket in the same rollout that turns on string emission.)

**Skip rules.** The Swift walker's standard directory skips (dotdirs, `node_modules` / `build` / `dist` / `coverage` / `DerivedData` / `Pods`), plus a post-walk filter unless `--include-tests` is passed. That filter is the contract's normative test-path pattern set (see "Test path patterns" above) **unioned with** the legacy basename-substring markers `.test.` / `.spec.` (matched anywhere in the basename, not just as a suffix) that this catalog's pre-#317-alignment implementation already excluded — the union keeps this catalog's exclusion set a strict superset of what shipped before, so aligning Swift's `is_test` tag with the contract's suffix-only rule doesn't newly admit a file like `Legacy.test.helpers.swift`. Note the polarity: tests are *excluded by default* here, unlike the `type` and `func` catalogs — this is an occurrence catalog that deliberately omits per-row `is_test` tagging, so exclusion has to happen at extraction time instead.
