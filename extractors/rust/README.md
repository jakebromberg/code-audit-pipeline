...
- **`symbol_id`** reuses the type catalog's 4-tuple `(package, file, name, kind)` sha1 (the Python function catalog adds `signature_index`, always `0` here). It matches `symbol-id-collisions.jq`'s grouping but is **not** guaranteed unique: two same-named methods split across multiple `impl` blocks on one type — or same-named items in nested modules — collide, the same bare-name limitation the type catalog carries (see Known limitations).
- **Nested fns are not emitted as rows** (mirrors the type catalog's no-fn-body-items limitation). Their text *is* still part of the enclosing fn's `body_hash` (a fn body is rendered statement-by-statement, including any item-statements), matching the Python reference's `ast.unparse` behavior.
...
- **Bare (unqualified) declaration names.** Items nested in modules are emitted under their bare identifier (not module-qualified), so two same-named types — or two same-named methods split across multiple `impl` blocks on one type (`A.run` from two `impl A`) — share a `symbol_id` within a file. The dominant top-level / single-impl case is unaffected.
- **Extern-block fns not catalogued.** Foreign function declarations inside `extern "C" { … }` (FFI imports, bodyless) are not emitted — they are declarations of non-Rust functions, out of scope for body-duplication and this crate's own surface.
...
- **Raw identifiers in type/field positions keep the `r#` prefix.** A `r#type` struct/field is catalogued verbatim (`r#type`) in the type catalog. (Function-catalog *parameter* names are unraw'd — `r#type` -> `type`.) Raw idents are rare in type/field positions.
