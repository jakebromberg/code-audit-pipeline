...
#
# Optional --catalog-kind scope: when CROSS_REPO_CATALOG_KIND is set in
# the env (the wrapper sets it from its --catalog-kind flag), only
# catalogs whose `.kind` matches contribute. Without this filter, version
# skew in a kind the user is not even querying would refuse the run.
#
...
# not a stack trace. `.latest` of a non-object type is treated as null
# (no catalogs contributed).
def catalog_rows:
  ...
  | [
      ...
      | .repo as $repo
      | (.latest.catalogs // [])[]
      | select($required_kind == null or .kind == $required_kind)
      | (
