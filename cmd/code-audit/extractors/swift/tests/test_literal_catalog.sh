...
# Exact record inventory: 13 rows (see fixture comments). An accidental new
# emission position (tuple elements, attribute args, raw values, …) surfaces
# here first.
assert_jq "total record count" '13' 'length' "$OUT"
...
assert_jq "string literal not emitted" '0' \
    '[.[] | select(.value=="\"6\"" or (.binding_name // "")=="text")] | length' "$OUT"
...
assert_jq "every row has core fields" '13' \
    '[.[] | select(.package and .file and (.line | type=="number") and .value and .value_norm and (.value_kind=="int" or .value_kind=="float") and (.form=="binding" or .form=="argument"))] | length' "$OUT"
