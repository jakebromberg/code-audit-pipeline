...
Emits one record per function-like construct that has a body: top-level `func`, methods, initializers, deinitializers, subscripts, computed-property getters/setters. Skipped: protocol-requirement signatures (no body) — that's it. `--min-body-lines` (default 3) no longer skips rows; functions whose normalized body has fewer lines than the threshold still emit a row, with `body_hash` / `body_lines` / `body_line_count` / `body_length` all `null` instead of being dropped, so row-level and count-based consumers (e.g. `module-symbol-density.jq`) now see one-liners and trivial accessors that used to be invisible; body-level clustering still correctly skips them via `select((.body_line_count // 0) >= 3)`. This does not improve recall for signature-level queries — `public-api-leaks.jq` gates on `.params[].type_ref` and `.return_ref`, and Swift's function-catalog emits neither field, so it returns zero rows against Swift both before and after this change (tracked separately as #328).
...
`enclosing_type` is the dotted nesting-stack qualification of the row's enclosing declaration (`Outer.Inner`), the extended type's text for extension members (`Array<Concert>`), and `null` for free functions — the same qualification `type`'s nested-type naming produces. It is not by itself a safe join key: two package roots can carry a same-named (or same-nested-name) type, so joining function-catalog rows onto type-catalog rows requires `(package, enclosing_type) == (package, name)`, not a bare `enclosing_type == name`. That join is still one-to-many, not exact: a declaration and its extensions share `(package, name)` — `protocol Greeter {}` plus `extension Greeter { ... }` both emit rows keyed `(package, "Greeter")`, one `kind: "interface"` and one `kind: "extension"` — so a consumer must disambiguate on `.kind`, and a match may be extension-only (e.g. `extension Array<Concert>`, where `Array` is never declared in-package and no declaration row exists at all; the extension-only row carries neither `fields` nor `shape_sig`). See `docs/pipeline-contract.md`'s function-catalog section for the full rationale.
