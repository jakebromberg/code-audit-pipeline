...
- **`inter_rater_collapsed`** — Fleiss κ over distinct (plant_id, cluster_id) judgments × N raters (per-(reviewer, judgment) median of the duplicates). Cleaner for round-1's correlated set; low statistical power (N=1 judgment after the round-2 gate, so collapsed κ is degenerate and the sentinel persists even at m≥2 unless a future round introduces a second distinct judgment).
