...
- **Plant 1R restraint false-positive is binding-structural, not prompt-behavioral.** Round-1 audit shows the agent already emits `no-action` with `reason_class: "sample-app-mirror"` on Plant 1R's two canonical clusters (`exact-duplicates:MetricRow+MetricRow` and `function-duplicates-exact:...MetricRow.body...`). The 1.0 per-cell FPR comes from incidental bindings — `DebugMetricsProvider` clusters in `DebugHUD.swift` (Plant 1R's source file) that aren't about MetricRow. The round-1 closeout shipped a prompt edit (rule 1 of the agent prompt now treats `is_test=true | is_mock=true | is_sample_app=true` on ANY participating record as load-bearing, not contingent on ALL records) as a methodological correction whose measurable effect lands in round 2. The structural FPR persists in round 1; resolution is the same §6.2 symbol-level matching that fixes the §4.2 binding artifact.
...
- **Substring-match binding produces false positives, partially mitigated.** The round-1 closeout's prefer-signal-match rule in `bind_recs_to_plants` cleans up 24 incidental false bindings (cases where one plant signal-matches and another doesn't). The HSBColor panel-routed Plant 3.1 / Plant 5.1 co-binding is NOT cleaned up — both plants legitimately share the `function-duplicates` signal AND HSBColor.swift as a source file. Symbol-level matching is the round-2 fix; see [§4.2](#42-binding-artifact-finding).
