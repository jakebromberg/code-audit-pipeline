...
- <a name="glossary-primary-match-specifics-blessed-alternative"></a>**`primary_match_specifics_blessed_alternative`.** All required specifics keys matched, with at least one matched via a pre-blessed alternative in `plant.primary_answer.specifics_alternatives[key]` rather than the canonical value. **Scores 1.0; does NOT route to panel.** Added in round 4 ([PR #112](https://github.com/jakebromberg/code-audit-pipeline/pull/112)) for the H0b sub-experiment ([results.md §11](results.md#11-rubric-loosening-sub-experiment-round-4--h0b-not-supported)). The new label is for telemetry and auditability — readers can tell which scored rows benefited from the H0b fall-through without re-reading the manifest.
...
- <a name="glossary-round-4"></a>**Round 4.** 2026-05-21 / 2026-05-25. [Rubric-loosening sub-experiment (results.md §11)](results.md#11-rubric-loosening-sub-experiment-round-4--h0b-not-supported). Adds `specifics_alternatives` to the per-plant manifest schema and a fall-through to the auto-scorer's `_specifics_values_match` so blessed-alternative matches score 1.0 under the new [`primary_match_specifics_blessed_alternative`](#glossary-primary-match-specifics-blessed-alternative) label. Outcome: H0b not supported (0.0% relative drop on both denominators; 4 of 47 alternatives exercised at the per-key level but no row promoted because at least one other required key on each row diverged from both the canonical and the blessed alternatives).
