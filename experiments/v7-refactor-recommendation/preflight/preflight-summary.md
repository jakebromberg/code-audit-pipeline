...
1. **Pin the Sonnet date-suffix.** The API returned bare `claude-sonnet-4-6`; methodology §10 requires a date-pinned identifier. Update `reproducibility.yaml > execution.model_versions.primary_pinned_at` before the Phase D harness runs.
