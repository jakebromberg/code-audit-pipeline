...
| `field-copy-mapper-candidates.jq` | Hand-written field-by-field mappers (`Dest(x=src.x, …)` / `dst.x = src.x`) keeping two catalogued types in sync — `from_X` constructor + field-parity-test candidates | function + type |
