...
- Source build: prefer `./install.sh` — it runs `go generate ./...` first (so the embeds match the checkout's canonical sources, not the last-committed copies) and stamps `internal/cli.Version` with `git describe` of the checkout, which the Go toolchain's own VCS stamp gets wrong inside linked worktrees (it attributes the build to the parent checkout's HEAD, `+dirty`). Raw `go install ./cmd/code-audit` still works from a clean checkout; the `//go:embed` targets under `cmd/code-audit/{queries,extractors}/` are committed to the repo. `go install github.com/jakebromberg/code-audit-pipeline/cmd/code-audit@<tag>` works against the Go module proxy for the same reason. Tests: `bash pipeline/_tests/test_install.sh`.
