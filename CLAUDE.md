...
- Source build: `go install ./cmd/code-audit` works directly from a clean checkout; the `//go:embed` targets under `cmd/code-audit/{queries,extractors}/` are committed to the repo. `go install github.com/jakebromberg/code-audit-pipeline/cmd/code-audit@<tag>` works against the Go module proxy for the same reason.
- The canonical sources for embed content are `pipeline/queries/*.jq` and `extractors/<lang>/*`. The generated copies under `cmd/code-audit/{queries,extractors}/` are produced by `go generate ./...` (driven by `internal/genembed`) and **must be regenerated and committed in the same PR** whenever a canonical source changes. CI enforces this with `git diff --exit-code -- cmd/code-audit/queries cmd/code-audit/extractors` after running `go generate ./...`.
